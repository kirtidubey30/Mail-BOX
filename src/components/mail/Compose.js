...
const Compose = () => {
  ...
  const compose = () => {
    if (emaildata.to.length > 0 && emaildata.message.length > 0) {
      fetch("https://mailbox-9ba1a-default-rtdb.firebaseio.com/email.json", {
        method: "POST",
      ...
      window.alert("Fileds cannot be empty");
    }
  };
  if (!localStorage.getItem("email")) {
    ...
    return (
      <div class="flex md:h-screen bgColor flex-col emailDetail">
        <h1 className="inboxHeading">Compose Email</h1>
        <div className="w-[70%] m-auto  top-2rem ">
          ...
          <textarea
            ...
            id="message"
            rows="4"
            className=" ComposeText block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
